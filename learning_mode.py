...
class LearningAudioGenerator:
    def __init__(self):
        self.temp_dir = get_temp_path('audio_files')
        
    def generate_learning_audio(self, text, filename):
        """Генерирует аудио файл с произношением + тишина для повторения"""
        try:
    ...
    def concat_audio_files(self, audio_files, output_path):
        ...
        concat_list = get_temp_path("audio_concat_list.txt")
...
class LearningVideoGenerator:
    ...
    def create_learning_video(self, word_data, audio_duration, output_path):
        """Создает видео для одного слова с точной длительностью под аудио"""
        print(f"Создаем видео для слова: {word_data['thai']} (длительность: {audio_duration:.2f}с)")
...
def generate_learning_video(words_data, output_path):
    """Основная функция для создания видео в режиме заучивания"""
    print("🎓 РЕЖИМ ЗАУЧИВАНИЯ")
    print("="*50)
    
    # Генерируем аудио для каждого слова
    audio_generator = LearningAudioGenerator()
    video_generator = LearningVideoGenerator()
    ...
    for i, (word, duration) in enumerate(zip(words_data, audio_durations)):
        ...
        audio_file = get_temp_path('audio_files', f"learning_word_{i+1}.mp3")
    ...
    print("\n🔗 Объединяем видео...")
    concat_list_file = get_temp_path("learning_concat_list.txt")
    
    # Создаем список с абсолютными путями
    ...
    if os.path.exists(concat_list_file):
        ...
        print(f"✅ Видео в режиме заучивания создано: {output_path}")
        video_created = True
    ...
    return output_path if video_created else None